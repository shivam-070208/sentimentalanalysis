...
<Dashboardlayout role={"Student"}>
   <div className='w-full flex justify-between flex-wrap'>
    ...
    <div className='flex gap-3 flex-wrap justify-center'>
      <button className=' rounded px-3 py-1 gap-2 flex bg-blue-600 hover:bg-blue-500 cursor-pointer text-white  items-center'><span className='text-xl'>
       ☆ </span><span className='text-sm font-semibold'>
        Rate Teachers
        </span></button>
        <button className=' rounded px-3 py-2 bg-white border-1 border-neutral-300 text-sm cursor-pointer font-semibold hover:bg-green-500 hover:text-white'>Provide Suggestion</button>
    </div>
   </div>
   ...
   <div className='grid md:grid-cols-2 grid-cols-1 gap-7 w-full '>
    ...
    <p className='text-xs text-zinc-400'>Your Recent Feedback Submisions</p>

      < div className="space-y-4 mt-7 h-60 snap-y  overflow-auto ">
                {recentActivity.map((activity) => (
                  <div key={activity.id} className="flex items-start pb-4 border-b border-gray-100 snap-start">
    ...
    <p className='text-xs text-zinc-400'>Teachers you still need to rate</p>
       ...
       <div className="space-y-4 mt-7 h-60 snap-y  overflow-auto ">
                {pendingFeedbacks.map((feedback) => (
                  <div key={feedback.id} className="pb-4 border-b border-gray-100 snap-start">
