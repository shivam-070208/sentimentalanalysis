  ...
  return (
    <div className='w-full bg-white rounded pb-10 group overflow-hidden h-full flex items-center  flex-col'>
      ...
      <p className='text-zinc-400 text-sm mt-2'>{item=="Student"?'Rate your teachers and provide anonymous feedback':'View analytics and insights from student feedback'}</p>
       ...
       <button onClick={()=>sboard(item)}  className={`${item=="Student"?'bg-blue-700':'bg-green-600'} cursor-pointer w-fit mt-14 text-white text-md px-9 py-2 rounded-sm`}>Continue as {item}</button>
    </div>
  )
}
