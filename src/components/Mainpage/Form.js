...
const Form = ({board}) => {
  ...
  return (
    <div className='w-full max-w-lg m-auto bg-white flex mt-4 rounded flex-col py-3 items-center'>
      <h1 className='text-2xl font-semibold'>{board} Access</h1>
      <p className='text-zinc-400 text-sm '>Login or create a new account</p>
      <div className='px-2 py-1 rounded bg-zinc-100 flex mt-3'>
        {['Login','Signup'].map((item,i)=>(
            <div onClick={()=>sindex(i)} key={i} className={`px-22 z-1 relative text-sm ${index == i?'text-black':'text-zinc-500'} cursor-pointer font-semibold py-2`}>{item}
